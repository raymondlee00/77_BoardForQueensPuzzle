...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        for (int i = 0; i < lastRankFilled; i++) {
          if (filesWithQueens[lastRankFilled] == filesWithQueens[i] || (Math.abs(filesWithQueens[lastRankFilled] - filesWithQueens[i]) == lastRankFilled - i)) {
            return true;
          }
        }
        return false;
    ...
    public boolean accept() {
        if (ranks() <= 1) { //pseudo-solution LMAO
          return true;
        }
        if (lastIsNg()) {
          return false;
        ...
        for (int queenFilePosition : filesWithQueens) {
          if (queenFilePosition != 0) {
            isAllZero = false;
          }
        }
        return !isAllZero;
